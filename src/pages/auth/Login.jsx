...
export default function Login() {
    ...
    try {
      ...
      if (userId) {
        ...
        if (user) {
          ...
          // 3. Store tokens and basic info
          localStorage.setItem('token', accessToken);
          localStorage.setItem('admin_user', JSON.stringify(user));
          if (user.organizationId) localStorage.setItem('organizationId', user.organizationId);
          if (user.client) localStorage.setItem('client', user.client);
          
          // Save role-based and account-based flags
          localStorage.setItem('isMainAdmin', user.isAdmin ? 'true' : 'false');
          localStorage.setItem('isSubscribed', user.isSubscribed ? 'true' : 'false');
          localStorage.setItem('isPlatformAdmin', user.isPlatformAdmin ? 'true' : 'false');
          ...
          if (user.roleId) {
            ...
            if (role) {
              ...
              const featuresPermissions = permissions.map(p => {
                const featureName = p.feature_name || p.feature_key;
                const scopes = Object.keys(p.permissions).filter(k => p.permissions[k]).map(k => k.toUpperCase());
                return { featureName, permissionScopes: scopes };
              });
              localStorage.setItem('FeaturesPermissions', JSON.stringify(featuresPermissions));

              // Flatten permissions into localStorage
              permissions.forEach(p => {
                const featureKey = p.feature_key || p.feature_name;
                if (featureKey) {
                  const key = `${featureKey}Permissions`;
                  const scopes = Object.keys(p.permissions).filter(k => p.permissions[k]).map(k => k.toUpperCase());
                  localStorage.setItem(key, JSON.stringify(scopes));
                  // Store the raw object as well for PermissionWrapper's alternate lookup
                  localStorage.setItem(`${featureKey}PermissionsObj`, JSON.stringify(p.permissions));
                }
              });
              localStorage.setItem('admin_permissions', JSON.stringify(permissions));
              localStorage.setItem('role_scopes', JSON.stringify(permissions.map(p => ({ feature: p.feature_key, scope: p.data_scope }))));

              // Prioritize isCollege from user object (Source of Truth)
              const isCollege = (user.isCollege !== undefined && user.isCollege !== null) 
                ? !!user.isCollege 
                : role.code === 'ADMIN';
                
              localStorage.setItem('isCollege', isCollege ? 'true' : 'false');
              user.isCollege = isCollege;
            }
          }
          ...
          if (user.organizationId) {
            try {
              let isCollege = role?.code === 'ADMIN'; // Initial hint from role
              let orgRes = null;
              
              // NEW: Fetch basic organization info (smart inference happens in backend)
              const infoRes = await authAPI.getOrganizationInfo(user.organizationId);
              if (infoRes?.data?.success && infoRes?.data?.data) {
                const orgInfo = infoRes.data.data;
                
                // Prioritize backend inference
                if (orgInfo.isCollege !== null && orgInfo.isCollege !== undefined) {
                    isCollege = orgInfo.isCollege;
                }

                if (orgInfo.name) {
                  localStorage.setItem('collegeName', orgInfo.name);
                  user.orgName = orgInfo.name;
                }
              }

              // Update local state and storage with refined isCollege
              // FORCE isCollege to false for ATS role regardless of inference
              if (user.roleCode === 'ATS') isCollege = false;

              localStorage.setItem('isCollege', isCollege ? 'true' : 'false');
              user.isCollege = isCollege;

              // Fetch details based on confirmed type
              if (isCollege) {
                try { 
                    orgRes = await authAPI.getCollegeDetails(user.organizationId);
                } catch(e) { console.warn('College details not found'); }
              } else {
                try {
                    orgRes = await authAPI.getCompanyDetails(user.organizationId);
                } catch(e) { console.warn('Company details not found'); }
              }
  ...
  /* ── shared input class ── */
  const inputCls = 'w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-800 placeholder-slate-400 outline-none transition-all focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100';
      ...
      <div className="flex flex-1 items-center justify-center bg-white px-6 py-12 lg:px-16">
        <div className="w-full max-w-md">
          ...
          <div className="mb-8 flex items-center gap-3 lg:hidden">
            ...
            <span className="text-sm font-semibold tracking-widest uppercase text-[#0051A6]">KareerGrowth Admin</span>
          </div>
          ...
          <div className="w-full max-w-sm rounded-2xl bg-white p-7 shadow-2xl ring-1 ring-slate-200">
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full rounded-full bg-gradient-to-b from-blue-600 to-blue-700 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/30 ring-1 ring-inset ring-white/20 transition hover:-translate-y-0.5 hover:shadow-blue-500/40 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-60"
              >
