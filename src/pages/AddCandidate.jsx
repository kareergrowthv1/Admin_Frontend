...
const AddCandidate = () => {
    ...
    useEffect(() => {
        try {
            ...
            if (storedOrgId) {
                ...
                fetchCollegeDetails(storedOrgId);
            }
        } catch (err) {
    ...
    const fetchCollegeDetails = async (orgId) => {
        try {
            ...
            console.warn('Failed to fetch college/company details:', error);
        }
    };
    ...
    const fetchQuestionSets = async (positionId) => {
        try {
            const response = await axios.get('/candidates/question-sets', {
                params: {
                    positionId,
                    page: 0,
                    size: 1000
    ...
    const autoFetchCandidate = async (email) => {
        ...
        try {
            ...
            if (isMounted.current) setExistingCandidateId(null);
        } finally {
    ...
    const handleSubmit = async (e) => {
        ...
        // ──────────────────────────────────────────────────────────────────
        // STEP 1 (only if new): POST /candidates/add (Candidate Backend)
        ...
        if (!candidateId) {
            try {
                ...
                const candidatePayload = {
                    ...
                    mobileNumber: digitsOnly.length >= 10 ? digitsOnly.slice(-10) : digitsOnly,
                    createdBy: userId,
                    organizationId: organizationId || ''
                };
                const candidateBlob = new Blob([JSON.stringify(candidatePayload)], { type: 'application/json' });
                formDataToSend.append('candidate', candidateBlob);
                if (formData.resume_file) {
                    formDataToSend.append('resumeFile', formData.resume_file);
                }

                const step1Res = await candidateApi.post('/candidates/add', formDataToSend, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
                const step1Data = step1Res.data;
                candidateId = step1Data?.id;
                if (!candidateId) {
                    throw new Error('Response missing candidate id');
                }
                console.log('✅ Step 1 (candidates/add) OK:', JSON.stringify({ candidateId }));
            } catch (err) {
        ...
        <div className="space-y-0">
            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2 text-[11px] font-semibold text-slate-400">
                    ...
                    <span className="mx-1 text-slate-200">/</span>
                    <span className="text-slate-800 font-bold">Add Candidate</span>
                </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="grid grid-cols-2 gap-4 mb-5">
                    ...
                    <div className="relative" ref={questionSetDropdownRef}>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent text-sm cursor-pointer shadow-[inset_0_0_3px_rgba(0,0,0,0.1)]"
                            placeholder={
                            ...
                            <div className="absolute z-10 w-full mt-1 bg-white border border-slate-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                {questionSets.map(qs => (
                                    <div
                                        ...
                                        className={`px-4 py-2.5 text-sm cursor-pointer hover:bg-orange-50 transition-colors ${formData.question_set_id === qs.id ? 'bg-orange-100 font-semibold' : ''
                                            }`}
                                    >
                                        {qs.code || qs.id}
                                    </div>
                ...
                <div className="space-y-5">
                    <h3 className="text-[13px] font-bold text-slate-800 mb-4">Candidate Details (for Private Link)</h3>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    setFormData({ ...formData, candidate_email: emailValue });
                                    if (!emailValue || emailValue.trim() === '') setExistingCandidateId(null);
                                ...
                                className={`w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent text-sm shadow-[inset_0_0_3px_rgba(0,0,0,0.1)] ${isFetchingEmail ? 'opacity-50' : ''}`}
                    ...
                    {/* WhatsApp Number and Resume - Same Row */}
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-[12px] font-semibold text-slate-700 mb-1.5 block">
                            ...
                            <input
                                ...
                                className={`w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent text-sm shadow-[inset_0_0_3px_rgba(0,0,0,0.1)] ${isWhatsAppUsed ? 'border-red-500 ring-1 ring-red-500' : ''}`}
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent text-sm shadow-[inset_0_0_3px_rgba(0,0,0,0.1)] resize-none"
